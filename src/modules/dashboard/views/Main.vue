<template>
  <div class="h-full">
    <div class="flex flex-1 flex-col lg:pa-64">
      <main class="flex-1 pb-8">
        <div class="mt-8">
          <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <h2 class="text-lg font-medium leading-6 text-gray-900">Zusammenfassung</h2>
            <div class="mt-2 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div v-for="card in cards" :key="card.name" class="overflow-hidden rounded-lg bg-white shadow">
                ...
                <div class="bg-gray-50 px-5 py-3">
                  <div class="text-sm">
                    <router-link :to="{ name: card.href, params: {id: 1 }}" class="font-medium text-blue-700 hover:text-blue-900">Anzeigen</router-link>
                  </div>
                </div>
              </div>
            </div>
          ...
          <h2 class="mx-auto mt-8 max-w-6xl px-4 text-lg font-medium leading-6 text-gray-900 sm:px-6 lg:px-8">Neuesten Rezepte</h2>
          ...
          <div class="shadow sm:hidden">
            ...
            <nav class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3" aria-label="Pagination">
              <div class="flex flex-1 justify-between">
                <a href="#" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-500">Mehr Rezepte</a>
              </div>
            </nav>
          </div>
                ...
                <div class="min-w-full overflow-hidden overflow-x-auto align-middle shadow sm:rounded-lg">
                  <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                      <tr v-for="transaction in recipes" :key="transaction.id" class="bg-white">
                  ...
                  <nav class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6" aria-label="Pagination">
                    <div class="flex flex-1 justify-between sm:justify-end">
                      <a href="#" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Mehr Rezepte</a>
                    </div>
                  </nav>
                </div>
              </div>
...
const cards = [
  { name: 'Rezepte', href: 'recipeMain', icon: ScaleIcon, amount: '30' },
